...
export default function Project({ projectsData, otherProjects }: any) {
  ...
  const recommendedProjects = processProjects(otherProjects).projects.filter((e: ProjectCard) => e.id != project.id);
      ...
      <Section bg="light">
        <Wrapper>
          <div className="flex items-start justify-between mt-[-3rem] flex-wrap">
            <div className="w-full md:w-[70%]">{documentToReactComponents(project.content, richTextOptions)}</div>
              ...
              {recommendedProjects.map((e: ProjectCard) => (
                <Link href={`/proiecte-si-programe/${e.slug}`} key={e.id}>
                  <div className="w-full bg-[#fff] p-5 mb-8 drop-shadow-md rounded-md hover:drop-shadow-xl transition-all hover:translate-y-[-1rem]">
                    <Image src={e.photo} width={400} height={400} alt="" className=" rounded-md mb-3 w-full h-auto" />
                    <div>{e.title}</div>
                  </div>
                </Link>
              ))}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const otherProjects = await client.getEntries({ content_type: "projectsPrograms", limit: 4 });
